...
{
    public delegate void OnNetworkMessageDelegate(
        nint dataPtr,
        ushort opCode,
        uint sourceActorId,
        uint targetActorId,
    ...
    public GameNetwork(IGameInteropProvider io, IPluginLog pluginLog)
    {
        this.pluginLog = pluginLog;
        unsafe
        {
            var onReceivePacketAddress = (nint)
                PacketDispatcher.StaticVirtualTablePointer->OnReceivePacket;
            processZonePacketDownHook =
                io.HookFromAddress<PacketDispatcher.Delegates.OnReceivePacket>(
                    onReceivePacketAddress,
                    this.ProcessZonePacketDownDetour
                );
    ...
    private unsafe void ProcessZonePacketDownDetour(
    ...
    )
    {
        ...
        foreach (var d in Delegate.EnumerateInvocationList(this.NetworkMessage))
        {
            ...
            catch (Exception ex)
            {
                ...
                pluginLog.Error(ex, "Exception on ProcessZonePacketDown hook. Header: " + header);
            }
        }
